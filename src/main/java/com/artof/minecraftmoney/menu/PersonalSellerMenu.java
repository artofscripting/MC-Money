    ...
    public PersonalSellerMenu(int containerId, Inventory playerInventory, BlockEntity blockEntity) {
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 3; col++) {
                int sellerSlot = col + row * 3;
                addSlot(new Slot(this.blockEntity, sellerSlot, 62 + col * 18, 17 + row * 18) {
                    @Override
                    public boolean mayPlace(ItemStack stack) {
                        return PersonalSellerMenu.this.blockEntity.canPlaceItemThroughFace(sellerSlot, stack, Direction.UP);
                    }
                });
            }
        }
